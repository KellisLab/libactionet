...
vector<vector<int> > build_graph_unweighted(int n, int m, int* I, int* J) {
  ...
  for (int ij = 0; ij < m; ij++) {
    int i = I[ij], j = J[ij];
    if (i >= n || j >= n) throw std::invalid_argument("i or j bigger than n");
...
vector<term> dijkstra(int n, int m, int* I, int* J, double* V) {
       ...
       source++)  // no need to do final vertex because i<j
  {
    ...
    vector<double> d(
        n, std::numeric_limits<double>::max());  // init 'tentative' distances
                                                 // to infinity
...
vector<double> schedule(const vector<term>& terms, int t_max, double eps) {
  ...
  vector<double> etas;
  etas.reserve(t_max);
  for (int t = 0; t < t_max; t++) etas.push_back(eta_max * exp(-lambda * t));
