...
namespace ACTIONet
{
  ...
  multilevel_archetypal_decomposition prune_archetypes(
      arma::field<arma::mat> C_trace, arma::field<arma::mat> H_trace, double min_specificity_z_threshold, int min_cells)
  {
    arma::mat C_stacked;
    arma::mat H_stacked;
    ...
    for (int i = 0; i < total_archs; i++)
    {
      ...
      if (0 < common_landmarks.n_elem)
      { // They don't agree on any samples!
        landmark_cells(i) = common_landmarks(arma::index_max(c(common_landmarks)));
      }
      else
      { // Potentially noisy archetype
        pruned(i) = 1;
        bad_archs++;
      }
    }
