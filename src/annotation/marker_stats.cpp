...
namespace actionet {
    ...
    arma::mat computeFeatureStatsVision(arma::sp_mat& G, arma::sp_mat& S, arma::sp_mat& X,
        ...
        // `X` is features(genes) x labels; formerly `marker_mat`
        // S is cells x genes (obs x var, Plan 02 contract).
        // S.n_rows = cells, S.n_cols = genes.
        ...
        // stats = S * X: (cells x genes)(genes x labels) = cells x labels
        arma::mat stats = arma::mat(S * X);

        // Compute cell-specific stats to adjust for depth, etc.
        // Iterate over cells x genes sparse matrix:
        //   it.row() = cell index, it.col() = gene index
        arma::vec mu = arma::zeros(S.n_rows);   // per-cell
        arma::vec nnz = arma::zeros(S.n_rows);  // per-cell
        for (arma::sp_mat::const_iterator it = S.begin(); it != S.end(); ++it) {
            mu[it.row()] += (*it);
            nnz[it.row()]++;
        }
        mu /= S.n_cols;            // mean over genes (n_cols = genes)
        arma::vec p_nnz = nnz / S.n_cols; // 1 - p_zero, per cell
        ...
        sigma_sq += (S.n_cols * (1 - p_nnz)) % arma::square(mu); // Adjust for zeros
        sigma_sq /= (S.n_cols - 1);
