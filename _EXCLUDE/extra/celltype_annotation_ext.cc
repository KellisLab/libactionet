...
arma::mat compute_marker_aggregate_stats_basic_sum_perm_smoothed(arma::sp_mat &G, arma::sp_mat &S,
    ...
    arma::mat stats = actionet::computeNetworkDiffusionFast(G, raw_stats, thread_no, alpha,
    ...
    mini_thread::parallelFor(
            0, perm_no, [&](size_t i) {
                ...
                arma::mat rand_stats = actionet::computeNetworkDiffusionFast(G, raw_rand_stats, 1, alpha,
...
arma::mat compute_marker_aggregate_stats_basic_sum_smoothed(arma::sp_mat &G, arma::sp_mat &S, arma::sp_mat &marker_mat,
    ...
    arma::sp_mat raw_stats = arma::trans(arma::sp_mat(X * S));
    arma::mat stats = actionet::computeNetworkDiffusionFast(G, raw_stats, thread_no, alpha, max_it) *
...
arma::mat compute_marker_aggregate_stats_basic_sum_smoothed_normalized(arma::sp_mat &G, arma::sp_mat &S,
    ...
    arma::vec pr = actionet::computeNetworkDiffusionFast(G, p, thread_no, alpha, max_it).col(0);
...
arma::mat compute_marker_aggregate_stats_basic_sum_perm_smoothed_v2(arma::sp_mat &G, arma::sp_mat &S,
    ...
    mini_thread::parallelFor(
            0, perm_no, [&](size_t i) {
                ...
                arma::mat rand_stats = actionet::computeNetworkDiffusionFast(G, raw_rand_stats, 1, alpha, max_it) *
...
arma::mat compute_marker_aggregate_stats_TFIDF_sum_smoothed(arma::sp_mat &G, arma::sp_mat &S, arma::sp_mat &marker_mat,
    ...
    if (alpha == 0) {
        ...
        stats = actionet::computeNetworkDiffusionFast(G, raw_stats, thread_no, alpha, max_it) *
                arma::diagmat(arma::vec(arma::trans(arma::sum(raw_stats))));
    }
...
arma::mat compute_markers_eigengene(arma::mat &S, arma::sp_mat &marker_mat, int normalization, int thread_no) {
    ...
    mini_thread::parallelFor(
            0, marker_mat.n_cols, [&](size_t i) {
                ...
                arma::field<arma::mat> SVD_results = actionet::svdHalko(subZ, 1, 5, 0, 0);
    ...
    arma::mat aggregate_genesets_mahalanobis_2archs(arma::sp_mat& G, arma::sp_mat& S, arma::sp_mat& marker_mat,
        ...
        if (pre_alpha != 0) {
            ...
            T = computeNetworkDiffusionApprox(P, T_t, thread_no, pre_alpha);
            T = arma::trans(T);
        }
        ...
        mini_thread::parallelFor(
            0, marker_mat.n_cols, [&](int j) {
                ...
                if (nnz_idx.n_elem != 0) {
                    ...
                    arma::field<arma::mat> AA_res = runAA(T_scaled, W0, 100);
                    arma::mat C = AA_res(0);
                    arma::mat H = AA_res(1);
        ...
        if (post_alpha != 0) {
            stdout_printf("Post-smoothing expression values ... ");
            marker_stats_smoothed = computeNetworkDiffusionApprox(P, marker_stats_smoothed, thread_no,
                                                                        post_alpha);
            stdout_printf("done\n");
            FLUSH;
        }
